...
Common questions about using **LazySheprd** (`lazysheprd`) with [Herdr](https://herdr.dev).
...
|---------------------|------------|-----|
| **TUI wizard** (`./bin/lazysheprd-tui`) | **Plain terminal** (iTerm, Terminal.app, WezTerm, …) | Full-screen curses works more cleanly outside Herdr’s chrome |
| **CLI** (`./bin/lazysheprd init …`) | Plain terminal | No UI needed |
...
| **`lazysheprd status`** | Either | Talks to the live Herdr server |
...
./bin/lazysheprd init --non-interactive --name acme --dir ~/projects/acme --yes

# TUI
# On the “Target directory” step, edit the default ($PWD/<name>)
```

**Tip:** if you run `./bin/lazysheprd-tui` from inside `lazysheprd`, the default project folder is created *inside that repo* unless you change the path.

---

## Do I have to run this from the lazysheprd repo?

**No — after `./install.sh`.**

```bash
cd lazysheprd && ./install.sh
# then from anywhere:
lazysheprd help
lazysheprd-tui
```

- Install symlinks `lazysheprd` / `lazysheprd-tui` / … into `~/.local/bin` (pointing at this checkout).
...
| Step | Where |
|------|--------|
| Run wizard / `lazysheprd init` | Plain terminal (recommended) |
...
| Entry | Use when |
|-------|----------|
| `./bin/lazysheprd-tui` | Guided visual setup (roles, archetype, git, layout, seed) |
| `./bin/lazysheprd init …` | Scriptable / non-interactive create |
| `./bootstrap.sh <name>` | Dumb copy of all agent files — no `team.yaml`, no archetypes, no layout |

Prefer **`lazysheprd-tui`** or **`lazysheprd init`**. Keep `bootstrap.sh` as the minimal fallback.
...
```bash
# Files only
./bin/lazysheprd init --non-interactive --name visual-files \
  --archetype greenfield-web --no-git --no-herdr-layout --yes

# Layout, no agents yet
./bin/lazysheprd init --non-interactive --name visual-layout \
...
```text
~/.config/lazysheprd/templates/*.json
```

```bash
./bin/lazysheprd template list
./bin/lazysheprd template dir
./bin/lazysheprd template save my-web --archetype greenfield-web
./bin/lazysheprd init --non-interactive --name acme --template my-web --yes
```
...
```bash
./bin/lazysheprd update ~/acme --disable design --yes
# CONVENTIONS.md is never overwritten by update
# agents/*.md are skipped unless you force:
./bin/lazysheprd update ~/acme --force-prompts --yes
```
...
Yes. The product logic lives in `lazysheprd/` (plan + materialize + layout + seed). The TUI is a thin skin (`lazysheprd/tui.py`). A Go/Charm or Rust/Ratatui front end can call the same flows later without rewriting packs/archetypes.
